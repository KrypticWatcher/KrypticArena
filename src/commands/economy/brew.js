...
export default {
  data: new SlashCommandBuilder()
    .setName('brew')
    .setDescription('Brew potions from herbs and Vials of Water for Herbalism XP')
    .addIntegerOption((opt) => opt.setName('type').setDescription('Which tier of herb to brew').setRequired(true).setAutocomplete(true))
    .addIntegerOption((opt) => opt.setName('quantity').setDescription('How many potions to brew this trip (defaults to what you have, up to your max trip size)')),
  ...
  async execute(interaction) {
    ...
    try {
      const result = await startHerbalismTrip(interaction.guildId, interaction.user.id, interaction.channelId, interaction.user.displayName, tier, quantity);
