...
export default {
  data: new SlashCommandBuilder()
    .setName('fletch')
    .setDescription('Fletch arrows, bows, rods, or Magic Staves for Fletching XP (requires a Knife)')
    .addStringOption((opt) => opt.setName('type').setDescription('Which arrows, bow, or rod to fletch').setRequired(true).setAutocomplete(true))
    .addIntegerOption((opt) => opt.setName('quantity').setDescription('How many to fletch this trip (defaults to what you have, up to your max trip size)')),
  ...
  async execute(interaction) {
    ...
    try {
      const result = await startFletchingTrip(interaction.guildId, interaction.user.id, interaction.channelId, interaction.user.displayName, itemType, key, quantity);
